...
=end
      ...
      def active_scaffold_search_range(column, options)
        ...
        html = []
        select_options = [:string, :text].include?(column.column.type) ? ActiveScaffold::Finder::StringComparators : ActiveScaffold::Finder::NumericComparators.collect {|comp| [as_(comp.titleize), comp]}
