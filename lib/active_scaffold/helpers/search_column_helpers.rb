  ...
  module Helpers
    ...
    module SearchColumnHelpers
      ...
      def active_scaffold_search_range(column, options)
        ...
        select_options = [:string, :text].include?(column.column.type) ? ActiveScaffold::Finder::StringComparators.collect {|title, comp| [as_(title), comp]} : ActiveScaffold::Finder::NumericComparators.collect {|comp| [as_(comp), comp]}
        html << select_tag("#{options[:name]}[opt]",
